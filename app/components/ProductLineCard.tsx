...
interface ProductLineCardProps {
  item: DrupalProductLine
}

export default function ProductLineCard({ item }: ProductLineCardProps) {
  return (
    <Link
      href={item.path || '#'}
    ...
    >
      ...
      <div className="p-6">
          {(item as any).leadTime && (
            <p className="text-sm text-slate-700 font-medium mb-2">{(item as any).leadTime}</p>
          )}
