...
async function getNews(path: string): Promise<DrupalNews | null> {
  try {
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const item = await getNews(path)

  if (!item) {
    return { title: 'News Not Found | Manufacturing' }
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            <div className="lg:col-span-2">
              {(item as any).newsImage?.url && (
                <div className="relative h-64 md:h-96 rounded-xl overflow-hidden shadow-lg mb-8">
                  <ResponsiveImage
                    src={(item as any).newsImage.url}
                    alt={(item as any).newsImage.alt || item.title}
                    fill
                    className="object-cover"
                    variations={(item as any).newsImage.variations}
                    targetWidth={800}
                  />
                </div>
              )}
