...
import productLinesData from '@/data/mock/products.json'
...
const mockDataMap: Record<string, any> = {
  ...
  'products.json': productLinesData,
  'facilities.json': facilitiesData,
  'certifications.json': certificationsData,
  'news.json': newsData,
...
export function handleMockQuery(body: string): any {
  try {
    ...
    if (query.includes('GetProductLines') || query.includes('nodeProductLines')) {
      return loadMockData('products.json')
    }

    if (query.includes('GetFacilities') || query.includes('nodeFacilities')) {
      return loadMockData('facilities.json')
    }

    if (query.includes('GetCertifications') || query.includes('nodeCertifications')) {
      return loadMockData('certifications.json')
    }
